...
(function() {
  var diff =
    d3.select("#inner-content").node().offsetTop +
    d3.select("#infographic").node().offsetTop;
  var scrollPad = window.innerHeight - diff - 0.15 * window.innerHeight;
...
loadData(function(data) {
  ...
  var xPos = { l: width / 4, m: width / 2, r: width * 0.75 };
    ...
    .on("tick", function(e) {
      ...
      data.forEach(function(d, i) {
        if (d.outtakePos) {
          ...
          d.y +=
            ((exploring ? (height * 1) / 2 : (height * 3.7) / 6) + -d.y) * k;
        } else {
          d.x += (width / 2 + -d.x) * k;
          d.y +=
            ((noGrouping ? (height * 1) / 2 : (height * 2.5) / 6) - d.y) *
            1.5 *
            k;
        }
      });
  ...
  node.append("circle").attr("r", ƒ("radius"));
  ...
  gs.on("active", function(index) {
    ...
    d3.selectAll("section.step")
      ...
      .style("opacity", function(d, i) {
        return i == index ? 1 : i == index + 1 ? 0.1 : 0.001;
      });
    ...
    if (!exploring) {
      ...
      labels
        ...
        .text(ƒ("text"))
        .attr("x", function(d) {
          return d.x ? d.x : xPos[d.pos];
        })
        .attr("y", function(d) {
          return d.y ? d.y : height - 3;
        })
  ...
  groupSel
    .classed("graphic-button-container", true)
    .selectAll("div")
    .data(groupings)
    .enter()
    .append("div")
  ...
  exploreSel.select("#type-ahead").on("keyup", function(e) {
    ...
    if (result.length > 0) {
      d3.selectAll(".node").style("opacity", function(d, i) {
        return result.indexOf(d) > -1 ? 1 : 0.2;
      });
    ...
    var menuVisible =
      d3.select(".tt-dropdown-menu").node().style.display == "block";
    if (this.value == "" || menuVisible) resultDiv.classed("show", false);

    if (d3.event.keyIdentifier == "Enter") $(".typeahead").typeahead("close");
  });
  ...
  var locale = new Bloodhound({
    ...
    local: $.map(data, function(d) {
      return {
        value: d.Locale == "Traditional Domestic" ? "Domestic" : d.Locale
      };
    })
  });
